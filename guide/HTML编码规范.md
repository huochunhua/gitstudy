...
#### [强制] 使用 `4` 个空格做为一个缩进层级,使用tab来进行缩进，可以在IDE里进行设置（4个空格宽度）。
...
使用 `id`、属性选择作为 hook 是更好的方式。(特殊情况下可以使用js（或j）-classname的命名方式)
...
#### [建议] 在循环处理模板数据构造表格时，若要求每行输出固定的个数，建议先将数据分组，之后再循环输出。测试
